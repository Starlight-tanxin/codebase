...
public class WXPayReport {
    ...
    private static final int DEFAULT_CONNECT_TIMEOUT_MS = 6 * WXPayConstants.SECOND;
    private static final int DEFAULT_READ_TIMEOUT_MS = 8 * WXPayConstants.SECOND;
    ...
    private WXPayReport(final WXPayConfig config) {
        ...
        // 添加处理线程
        executorService = Executors.newFixedThreadPool(config.getReportWorkerNum(), runnable -> {
    ...
    private static String httpRequest(String data, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        ...
        StringEntity postEntity = new StringEntity(data, WXPayConstants.DEFAULT_CHARSET);
        ...
        return EntityUtils.toString(httpEntity, WXPayConstants.DEFAULT_CHARSET);
    }
